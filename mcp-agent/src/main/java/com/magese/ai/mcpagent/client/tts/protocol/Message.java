package com.magese.ai.mcpagent.client.tts.protocol;
...
public class Message {
    ...
    public static Message unmarshal(byte[] data) {
        ...
        if (flag == MsgTypeFlagBits.POSITIVE_SEQ || flag == MsgTypeFlagBits.NEGATIVE_SEQ) {
            ...
            if (buffer.remaining() >= 4) {
                buffer.get(sequenceBytes); // Read 4 bytes into array
                ByteBuffer wrapper = ByteBuffer.wrap(sequenceBytes);
    ...
    public String toString() {
        return switch (this.type) {
            case AUDIO_ONLY_SERVER, AUDIO_ONLY_CLIENT -> {
                if (this.flag == MsgTypeFlagBits.POSITIVE_SEQ || this.flag == MsgTypeFlagBits.NEGATIVE_SEQ) {
                    yield String.format("MsgType: %s, EventType: %s, Sequence: %d, PayloadSize: %d", this.type, this.event, this.sequence,
                            this.payload != null ? this.payload.length : 0);
                }
                yield String.format("MsgType: %s, EventType: %s, PayloadSize: %d", this.type, this.event,
                        this.payload != null ? this.payload.length : 0);
            }
            case ERROR ->
                    String.format("MsgType: %s, EventType: %s, ErrorCode: %d, Payload: %s", this.type, this.event, this.errorCode,
                            this.payload != null ? new String(this.payload) : "null");
            default -> {
                if (this.flag == MsgTypeFlagBits.POSITIVE_SEQ || this.flag == MsgTypeFlagBits.NEGATIVE_SEQ) {
                    yield String.format("MsgType: %s, EventType: %s, Sequence: %d, Payload: %s",
                            this.type, this.event, this.sequence,
                            this.payload != null ? new String(this.payload) : "null");
                }
                yield String.format("MsgType: %s, EventType: %s, Payload: %s", this.type, this.event,
                        this.payload != null ? new String(this.payload) : "null");
            }
        };
    }
